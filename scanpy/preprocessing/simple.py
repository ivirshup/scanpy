...
def calculate_qc_metrics(adata, exprs_values="counts", feature_controls=(), 
    ...
    for i, n in enumerate(percent_top):
        obs_metrics["pct_{exprs_values}_in_top_{n}_features"] = proportions[:, i] * 100
    for feature_control in feature_controls:
        obs_metrics["total_{exprs_values}_{feature_control}"] = \
            X[:, adata.var[feature_control].values].sum(axis=1)
        obs_metrics["log1p_total_{exprs_values}_{feature_control}"] = \
            np.log1p(obs_metrics["total_{exprs_values}_{feature_control}"])
        obs_metrics["pct_{exprs_values}_{feature_control}"] = \
            obs_metrics["total_{exprs_values}"] / \
            obs_metrics["total_{exprs_values}_{feature_control}"]
